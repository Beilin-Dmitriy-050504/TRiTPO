...
int GetFile(){
	std::ifstream fin;
	int res;
	fin.open("Interface\\HighScore.bin", std::ios::binary);
	if (!fin.is_open()) 
		return 0;
	else
...
void SetFile(int HighScore){
	std::ofstream fout;
	fout.open("Interface\\HighScore.bin", std::ios::binary);
...
game_init:
	...
	if (!font->loadFromFile("Interface\\arial.ttf")) exit(0);
	...
	startScreen.loadFromFile("Interface\\startScreen.png");
	endScreen.loadFromFile("Interface\\endScreen.png");
	Tiles.loadFromFile("Interface\\TilesFinal.png");//вручную нарисовання картинка, с 7 разноцветными квадратиками
	Background.loadFromFile("Interface\\BackgroundFinal.png");
